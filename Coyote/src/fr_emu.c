...
{
   double xstar[p+1], ystar[2*fr_nsim], val, k_invMpc;
   int ihi, ilo, i;
   ...
   /* Check k-range */
   testErrorRetVA(k_invMpc < fr_ksim_min || k_invMpc > fr_ksim_max, coyote_range,
           "Fourier mode k=%g 1/Mpc out of range [%g;%g]",
           *err, __LINE__, -1.0, k_invMpc, fr_ksim_min, fr_ksim_max);
