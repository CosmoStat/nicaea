...
{
   cosmo_lens* model;
   double k, ell, theta, a, pk, pg, f, z, incr;
   int i_bin, j_bin, Nzbin, c, dimensionless, verbose, lin, header, iell;
   error *myerr = NULL, **err;
   char *theta_fname, *theta_info, *ell_info, *substr, *slinlog, *sOmega_m, *ssigma_8, *out_suf, out_name[1024];
   FILE *F;
   size_t Nrec, Ntheta, Nell;
   double *theta_list, Theta_min, Theta_max, ell_min, ell_max, ell_fac;
   double z_mean, z_median;
   int calc_P_delta, calc_n_of_z, calc_P_kappa_d, calc_P_kappa_c, calc_xi_pm, calc_gamma2, calc_map2;
...
end_arg:
   ...
   if (out_suf==NULL) {
      out_suf = (char*)malloc_err(512*sizeof(char), err);
...
#define Nspar 2
   ...
   if (calc_P_kappa_c) {
      ...
      if (model->projection != full) {
         ...
         sprintf(out_name, "%s%s", "P_kappa", out_suf);
         F = fopen(out_name, "w");
         write_header_p_kappa(F, header, model, dimensionless, Nzbin, err);
         quitOnError(*err, __LINE__, stderr);
