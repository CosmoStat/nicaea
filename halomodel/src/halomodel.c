/* ============================================================ *
 * halomodel.c							*
 * Martin Kilbinger, Jean Coupon 2006-2015                      *
 * Refs:							*
 *   - Takada & Jain 2003					*
 *   - Eisenstein & Hu 1998					*
 *   - Percival 2005						*
			     ...
			     hod_t HOD, double pi_max, error **err)
{
   ...
   res->redshift = init_redshift(Nzbin, Nnz, nofz, photz, par_nz, NULL, err);
   forwardError(*err, __LINE__, NULL);
...
//		  *err, __LINE__, NULL, M_min, pow(logMmax,10.0));
   ...
   res->log10M_min  = log10M_min;
   res->log10M0     = log10M0;
   res->eta         = eta;

   /* leauthaud11 and coupon15 models */
   res->log10Mstar0 = log10Mstar0;
   res->beta        = beta;  
   res->delta       = delta;
   res->gamma       = gamma;
   res->log10Mstar_min    = log10Mstar_min;
   res->log10Mstar_max    = log10Mstar_max;
   ...
   testErrorRet((res->hod == leauthaud11 || res->hod == coupon15) && res->log10Mstar_min < 0, hm_undef,
                "Minimum stellar mass cannot be set to a negative value (undef) for leauthaud11 and coupon15 models.",
                *err, __LINE__, NULL);
   testErrorRet((res->hod == leauthaud11 || res->hod == coupon15) && res->log10Mstar_max < 0, hm_undef,
                "Maximum stellar mass cannot be set to a negative value (undef) for leauthaud11 and coupon15 models.",
                *err, __LINE__, NULL);
   testErrorRet(res->log10Mstar_min > res->log10Mstar_max && res->log10Mstar_max > 0, hm_undef,
...
cosmo_hm* copy_parameters_hm_only(cosmo_hm* source, error **err)
{
   ...
   res = init_parameters_hm(source->cosmo->Omega_m, source->cosmo->Omega_de, source->cosmo->w0_de,
             ...
             source->redshift->photz, source->redshift->par_nz,
			    ...
			    source->massfct, source->halo_bias, source->log10M_min, source->log10M1, 
			    source->log10M0, source->sigma_log_M, source->alpha, 
			    source->log10Mstar0, source->beta, source->delta, source->gamma, source->B_cut, source->B_sat, 
			    source->beta_cut, source->beta_sat, source->log10Mstar_min, source->log10Mstar_max, source->eta,
   ...
   */
   ...
   switch (tmp->hod) {
      case berwein02 : case berwein02_hexcl: case hamana04 :
         ...
         CONFIG_READ(tmp, log10M_min, d, F, c, err);
         CONFIG_READ(tmp, log10M1, d, F, c, err);
         CONFIG_READ(tmp, log10M0, d, F, c, err);
      ...
      case leauthaud11:
         ...
         /* sample properties */
         CONFIG_READ(tmp, log10M1, d, F, c, err);
         CONFIG_READ(tmp, log10Mstar0, d, F, c, err);
         CONFIG_READ(tmp, beta, d, F, c, err);
         CONFIG_READ(tmp, delta, d, F, c, err);
         CONFIG_READ(tmp, gamma,d, F, c, err);
         CONFIG_READ(tmp, log10Mstar_min, d, F, c, err);
         CONFIG_READ(tmp, log10Mstar_max, d, F, c, err);
         CONFIG_READ(tmp, sigma_log_M, d, F, c, err);
...
void dump_param_only_hm(cosmo_hm* model, FILE *F)
{
   ...
   fprintf(F, "# %.3f %.3f % .3f % .3f % .3f %s(%d) %s(%d) %s(%d)\n",
   ...
   if (model->hod == berwein02_hexcl || model->hod == berwein02){
    fprintf(F, "# log10M_min log10M1 log10M0    slogM alpha m b h\n");
    fprintf(F, "# %6.2e %6.2e %6.2e %.3f %.3f\n",
	    model->log10M_min, model->log10M1, model->log10M0, model->sigma_log_M, model->alpha);
   }else if (model->hod == leauthaud11){
    fprintf(F, "# log10M1 log10Mstar0 beta delta gamma sigma_log_M B_cut B_sat  beta_cut beta_sat alpha fcen1  fcen2   log10Mstar_min log10Mstar_max  \n");
    fprintf(F, "# %.3f %.3f %.3f %.3f %.3f %.3f       %.3f  %.3f %.3f   %.3f    %.3f %.3f %.3f  %.3f         %.3f\n",
	    model->log10M1, model->log10Mstar0, model->beta, model->delta, model->gamma, model->sigma_log_M, 
	    model->B_cut, model->B_sat, model->beta_cut, model->beta_sat,
	    model->alpha, model->fcen1, model->fcen2, model->log10Mstar_min, model->log10Mstar_max); 
   }else if (model->hod == coupon15){
      fprintf(F, "# log10M1 log10Mstar0 beta delta gamma sigma_log_M B_cut B_sat  beta_cut beta_sat alpha fcen1  fcen2   log10Mstar_min log10Mstar_max  \n");
    fprintf(F, "# %.3f %.3f %.3f %.3f %.3f %.3f       %.3f  %.3f %.3f   %.3f    %.3f %.3f %.3f  %.3f         %.3f\n",
       model->log10M1, model->log10Mstar0, model->beta, model->delta, model->gamma, model->sigma_log_M, 
       model->B_cut, model->B_sat, model->beta_cut, model->beta_sat,
...
double dsigma_R_sqr_dR(cosmo_hm *model, double R, error **err)
{
   ...
   h = R/20.0;
   /* Problematic ... ? */
   a = sigma_R_sqr(model, R+h, err); 	 forwardError(*err, __LINE__, 0);
   b = sigma_R_sqr(model, R-h, err);	 forwardError(*err, __LINE__, 0);
   res = (a - b) / (2.0 * h);

   if (-res < 0.0) {
      fprintf(stderr, "dsigma_R_sqr_dR %g %g %g  %g %g %g\n", R, R+h, R-h, res, sigma_R_sqr(model, R+h, err), sigma_R_sqr(model, R-h, err));
   }

   /* The variance decreases with increasing scale R, so the derivative has to be negative */
...
double sigmasqr_M(cosmo_hm *model, double M, error **err)
{
   ...
   R = cbrt(3.0*M/(4.0*pi*model->cosmo->Omega_m*rho_c0));
...
double dsigma_m1_dlnM(cosmo_hm *model, double M, error **err)
{
   ...
   rhobar = model->cosmo->Omega_m*rho_c0;
...
double dn_dlnM(double M, void *params, error **err)
{
  ...
  res     = model->cosmo->Omega_m*rho_c0/(a*a*a*M) * nfn/nu * dnudlnM;

  testErrorRetVA(res<0.0, ce_negative,  "dn_dlnM=%g negative", *err, __LINE__, 0.0, res);
  testErrorRet(!isfinite(res), ce_infnan, "dn_dlnM inf or nan",  *err, __LINE__, 0.0);
 ...
 * TO DO:                                                                *
 * Implement Munoz-Cuartas et al. (2011)                                 */
...
double concentration(cosmo_hm *model, double Mh, double a, error **err)
{
   ...
   /* 
      ...
      //DEBUGGING
      // if (Mh > 1e12){
      //printf("Mh = %e, c1 = %f, c2=%f, delta=%f\n", Mh, c, pow(10.0, log10_c), (c-pow(10.0, log10_c))/c*100);
      //exit(-1);
      //}
...
double r_vir(cosmo_hm *model, double M, double a, error **err)
{
  ...
  //return cbrt((3.0/4.0)*M/(pi*Delta*rhocrit));  
}
...
double rho_halo(cosmo_hm *model, double r, double a, double Mh, double c, error **err){
  ...
  */
  ...
  if (x > xmin) {
    ...
    // [JEAN] shouldn't it be 1/xmin?
    
    result = pow(x, 2.0-model->alpha_NFW);
  }
  
  testErrorRet(!isfinite(result), ce_infnan, "inf or nan encountered", *err, __LINE__, 0);
 ...
 * M defined with respect to the *mean* density times Delta.
 * r in comoving units
 * See Wright & Brainerd (2000), Eqs 11-16.
 * change Delta in Delta_h()
 */

#define eps 1.0e-10
double DeltaSigma_WB2000(cosmo_hm *model, double r, const double a, const double Mh,  double c, error **err){
  
  double result, rvir, Omega_m, rhocrit, Delta;
...
double bias_tinker10(cosmo_hm *model, double M, double a, error **err)
{
   ...
   double y = log10(Delta_h(model, model->nmz_a, err));  forwardError(*err, __LINE__, 0.0);
